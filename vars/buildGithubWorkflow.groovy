// Triggers a GitHub Actions workflow of the given repository via the
// workflow_dispatch API and, unless wait:false is passed, waits for the run to
// finish so that its result is reflected in this Jenkins build's status.
//
// Usage from a pipeline job:
//     buildGithubWorkflow(repo: 'lsfusion/web-agent')          // build-all on main, wait for result
//     buildGithubWorkflow(repo: ..., workflow: 'linux.yml')    // another workflow
//     buildGithubWorkflow(repo: ..., ref: 'feature/foo')       // another branch
//     buildGithubWorkflow(repo: ..., wait: false)              // fire-and-forget
...
def call(Map args) {
    ...
    try {
        withCredentials([string(credentialsId: 'api.github.com.token', variable: 'TOKEN')]) {
            ...
            if (!waitForResult) {
                echo "${name} `${workflow}` was triggered (ref: ${ref}). ${workflowUrl}"
                // slack.message "${name} `${workflow}` was triggered (ref: ${ref}).\n${workflowUrl}"
                return
            }
            ...
            if (conclusion == 'success') {
                echo "${name} `${workflow}` finished successfully (ref: ${ref}). ${run.html_url}"
                // slack.message "${name} `${workflow}` finished successfully (ref: ${ref}).\n${run.html_url}"
            } else {
                // slack.warning "${name} `${workflow}` finished with '${conclusion}' (ref: ${ref}).\n${run.html_url}"
                error "${name} ${workflow} run concluded with '${conclusion}': ${run.html_url}"
            }
        }
    } catch (e) {
