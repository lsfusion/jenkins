!define IDEA_MAJORVERSION 2020.3
!define IDEA_VERSION ${IDEA_MAJORVERSION}.1

!define JDK_MAJORVERSION 11 ; used in version compare and idea SDK
!define JDK_VERSION ${JDK_MAJORVERSION}.0.9 ; log, idea SDK
;!define JDK_FOLDER "$ProgramFiles${ARCH}\ojdkbuild\java-${JDK_MAJORVERSION}-openjdk-${JDK_DISTRVERSION}"

!define PG_MAJORVERSION 13
!define PG_VERSION ${PG_MAJORVERSION}.1

!define TOMCAT_MAJOR_VERSION 9
!define TOMCAT_VERSION ${TOMCAT_MAJOR_VERSION}.0.21

!define JASPER_VERSION 6.12.2

!define ANT_VERSION 1.10.9

# LSFUSION_MAJOR_VERSION, LSFUSION_VERSION and VI_LSFUSION_VERSION will be added automatically before building installers
!define LSFUSION_MAJOR_VERSION 4
!define LSFUSION_VERSION 4.1
!define VI_LSFUSION_VERSION 4.1.0.0
!define MYCOMPANY_VERSION 1.0 
!define DOWNLOAD_MYCOMPANY_JAR "https://download.lsfusion.org/solutions/mycompany-1.0.jar"